...
func LoadConfig(confPath string, confName string) (*MergedConfig, error) {
...
func (c *MergedConfig) Int(option string) (result int, found bool) {
	...
	log.Panicln("Failed to parse config option", option, "as int:", err)
	return 0, false
}
...
func (c *MergedConfig) Bool(option string) (result, found bool) {
	...
	// If it wasn't an OptionError, it must have failed to parse.
	log.Panicln("Failed to parse config option", option, "as bool:", err)
	return false, false
}
